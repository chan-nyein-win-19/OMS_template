...
<div class="row">
    ...
    <div class="col-md-2">
      <select onchange="this.options[this.selectedIndex].value && (window.location= this.options[this.selectedIndex].value);" class="form-control" id="filterLeave">
          ...
          <option value="{{url("/leader/leaveRecord/filterLeave/Approved/$today")}}" @if($filtering=='Approved')selected @endif>Approved</option>
          <option value="{{url("/leader/leaveRecord/filterLeave/Denied/$today")}}" @if($filtering=='Denied')selected @endif >Denied</option>
...
<td>{{$leaveRecord->employee->fname}} {{$leaveRecord->employee->lname}} </td>
    ...
    <td><a href="{{url("/leader/leaveStatus/$leaveRecord->id/approve/$today/$filtering")}}" type="button" class="btn btn-success mb-2">Approve</a> <a href="{{url("/leader/leaveStatus/$leaveRecord->id/deny/$today/$filtering")}}" type="button" class="btn btn-danger mb-2">Deny</a></td>
        
        @elseif($leaveRecord->status=="Approved")
        <td><a href="{{url("/leader/leaveStatus/$leaveRecord->id/cancel/$today/$filtering")}}" type="button" class="btn btn-success">Cancel</a></td>
        @else
        <td><a href="{{url("/leader/leaveStatus/$leaveRecord->id/cancel/$today/$filtering")}}" type="button" class="btn btn-danger">Cancel</a></td>
        @endif 
